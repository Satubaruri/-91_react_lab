...
function App() {
  ...
  return (
    <div className="container mt-3">
      <button className="btn btn-success position-fixed top-0 end-0 mt-3 me-4" onClick={addTask}>
        Add New Task
      </button>

      {tasks.map((task, index) => (
        <div key={index} className="card mt-3" style={{ backgroundColor: index % 2 === 0 ? '#f8f9fa' : '#e9ecef', border: '1px solid #ced4da' }}>
          <div className="card-body">
            {task.editing ? (
              ...
              <div>
                ...
                />
                <button className="btn btn-warning me-2" onClick={() => updateTask(index, task.task, task.description)}>
                  Save
                </button>
              </div>
            ) : (
            ...
            <div className="float-right">
              <button className="btn btn-info mr-2" onClick={() => editTask(index)}>
                Edit
              </button>
